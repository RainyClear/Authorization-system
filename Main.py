...
def admin_authorize():
    ...
    if type == "append":
        ...
        with open(f"{file_path}/log/authorize.log", "w", encoding = "utf-8") as file:
            file.write(f"{log_data}\n[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(now_timestamp))}](余额 {balance_now} / {key_data[Authorization[7:]]["account_balance"]}) {key_data[Authorization[7:]]["name"]} 为 {name} 授权 {num} 秒".strip("\n"))
